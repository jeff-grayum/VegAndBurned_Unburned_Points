...
```{r}
s23_locs_veg_birds_online_fixed <- read_xlsx("/Volumes/Samsung_T5/BOBWHITE_DATA/Clean/SummerLocations/Summer2023/s23_locs_veg_birds_online.xlsx")

# Counting unique birds signaled within a burned area each day
birds_in_burned_areas_each_day23 <- s23_locs_veg_birds_online_fixed %>%
  dplyr::filter(burned == 1) %>%
  group_by(date) %>%
  summarize(birds_in_burned_area_each_day23 = n_distinct(band_numb))

# Joining into summer locs data
s23_locs_veg_birds_online_fixed <- s23_locs_veg_birds_online_fixed %>%
  left_join(birds_in_burned_areas_each_day23, by = "date")

# Handling NA values for birds_in_burned_area_each_day23
s23_locs_veg_birds_online_fixed$birds_in_burned_area_each_day23[is.na(s23_locs_veg_birds_online_fixed$birds_in_burned_area_each_day23)] <- 0
...
s23_locs_veg_birds_online_fixed$percent_in_burned[is.na(s23_locs_veg_birds_online_fixed$percent_in_burned)] <- 0

#shortening name of df
s23rx <- s23_locs_veg_birds_online_fixed
```
