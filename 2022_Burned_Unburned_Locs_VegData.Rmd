...
set.seed(123)
trainIndex <- createDataPartition(summer_locs_veg_data_perc_burned_modified$percent_in_burned, p = .8, list = FALSE)
trainData <- summer_locs_veg_data_perc_burned_modified[trainIndex,]
testData  <- summer_locs_veg_data_perc_burned_modified[-trainIndex,]

# Example code for fitting a binomial model on training data
fit_binomial <- glm(cbind(birds_in_burned_area, birds_online - birds_in_burned_area) ~ pg + ps + hg + hs + DSF,
                    data = trainData, family = binomial, na.action = na.pass)

# k-fold cross-validation (e.g., 10-fold)
ctrl <- trainControl(method = "cv", number = 10)

train_binomial <- glm(cbind(birds_in_burned_area, birds_online - birds_in_burned_area) ~
                     pg +
                     ps +  
                     hg + 
                     hs +
...
plot(predictions - (testData$birds_in_burned_area)/testData$birds_online)
mtext("Binomial: Predictions - Observed values", outer = TRUE, line = -1, cex = 1.5)

mean_binomial <- mean(predictions - (testData$birds_in_burned_area)/testData$birds_online)
```
...
mean_beta <- mean(predictions - (testData$birds_in_burned_area)/testData$birds_online)

```

Let's check the normal distribution, too.
```{r}
# Partitioning data into training (80% ) and testing sets 
set.seed(123)
trainIndex <- createDataPartition(summer_locs_veg_data_perc_burned_modified$percent_in_burned, p = .8, list = FALSE)
...
# Example code for fitting a binomial model on training data
fit_normal <- glm(percent_in_burned ~  hg + pf + hs + DSF,
                    data = trainData, na.action = na.pass)

# k-fold cross-validation (e.g., 10-fold)
ctrl <- trainControl(method = "cv", number = 10)

train_normal <- glm(percent_in_burned ~
                     hg +
                     pf +
                     hs +
                     DSF,
...
# Evaluating the model on the test set, creating a confusion matrix
predictions <- predict(train_normal, newdata = testData, type = "response")


plot(predictions - (testData$birds_in_burned_area)/testData$birds_online)
mtext("Normal: Predictions - Observed values", outer = TRUE, line = -1, cex = 1.5)

mean_normal <- mean(predictions - (testData$birds_in_burned_area)/testData$birds_online)
```
