...
model_names <- c(paste0("gmodel", 1:10))


model_list <- list(gmodel1, gmodel2, gmodel3, gmodel4,
                   gmodel5, gmodel6, gmodel7, gmodel8, gmodel9, gmodel10)
...
##### Now for DSF predict ####
DSF_values <- seq(min(s22_rx_uniq_temp_mod$DSF, na.rm = TRUE), 
                 max(s22_rx_uniq_temp_mod$DSF, na.rm = TRUE), 
                 length.out = 1000)

DSF_df <- data.frame(
  hs = rep(mean(s22_rx_uniq_temp_mod$hs), length(DSF_values)),
  hg = rep(mean(s22_rx_uniq_temp_mod$hg), length(DSF_values)),
  hf = rep(mean(s22_rx_uniq_temp_mod$hf), length(DSF_values)),
  pg = rep(mean(s22_rx_uniq_temp_mod$pg), length(DSF_values)),
  pf = rep(mean(s22_rx_uniq_temp_mod$pf), length(DSF_values)),
  ps = rep(mean(s22_rx_uniq_temp_mod$ps), length(DSF_values)),
  DSF =  DSF_values
)
