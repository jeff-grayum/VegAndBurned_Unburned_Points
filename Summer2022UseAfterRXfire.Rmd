...
birds_in_burned_areas <- summer2022_locs_veg_data %>%
  ...
  group_by(date) %>%
  summarize(birds_in_burned_areas = n())
...
Candidate models w beta distribution (post MCM meeting)
```{r}
...
```{r}
#### ranking beta models (post meeting) ####
...
Making plots: Veg metrics vs percent in burned
```{r}
...
#first, we will calculate the mean values for hg, hf, and hs. These will be held constant when making predict plots
mean_hg <- mean(veg_data_2022$max_grasses_cm)
mean_hf <- mean(veg_data_2022$max_forbs_cm)
mean_hs <- mean(veg_data_2022$max_shrubs_cm)
...
hg_values <- seq(min(s22rx$hg, na.rm = TRUE), 
                 max(s22rx$hg, na.rm = TRUE), 
                 length.out = 1000) 
...
hg_mean <- betareg::predict(model5, newdata = hgdf, type = "response")
hg_var <- betareg::predict(model5, newdata = hgdf, type = "variance")

# Creating a new data frame with the calculated mean and variance
hg_results_df <- data.frame(
  hg_mean = hg_mean,
  hg_variance = hg_var
)

hg_results_df$lowCI = (hg_results_df$hg_mean - (1.96 * sqrt(hg_results_df$hg_variance)))
hg_results_df$upperCI = (hg_results_df$hg_mean + (1.96 * sqrt(hg_results_df$hg_variance)))

hg_results_df %>% view()

hg_results_df %>% 
  ggplot(aes(hg_values, hg_mean)) +
  geom_line() +
...
hf_values <- seq(min(s22rx$hf, na.rm = TRUE), 
                 max(s22rx$hf, na.rm = TRUE), 
                 length.out = 1000) 
...
hf_mean <- predict(model5, newdata = hfdf, type = "response")
hf_var <- predict(model5, newdata = hfdf, type = "variance")

hf_results_df <- data.frame(
  hf_mean = hf_mean,
  hf_var = hf_var
)

hf_results_df$lowCI = (hf_results_df$hf_mean - (1.96 * sqrt(hf_results_df$hf_var)))
hf_results_df$upperCI = (hf_results_df$hf_mean + (1.96 * sqrt(hf_results_df$hf_var)))

# Plotting
hf_results_df %>% 
  ggplot(aes(hf_values, hf_mean)) +
  geom_line() +
  geom_ribbon(aes(ymin = lowCI, ymax = upperCI), fill = "blue", alpha = 0.1) +
  labs(x = "Height of forbs (cm)",
       y = "Predicted % NOBO in burned areas",
...
#Now, we will do hs.
# Generate a sequence of hf values, between min and max.
hs_values <- seq(min(s22rx$hs, na.rm = TRUE), 
                 max(s22rx$hs, na.rm = TRUE), 
                 length.out = 1000) 
...
# Define a function to get betareg model predictions
boot_fn <- function(data, indices) {
  sample_data <- data[indices, ]
  mod <- betareg(percent_in_burned ~ hg + hf + hs, data = sample_data)
  return(predict(mod, newdata = hgdf))
}

# Create bootstrapped confidence intervals for hg
hg_boot <- boot(data = summer_locs_veg_data_perc_burned_modified, statistic = boot_fn, R = 1000)
hg_ci <- boot.ci(hg_boot, conf = 0.95, type = "perc")

# Calculate upper and lower bounds for hg
upr_hg <- hg_ci$percent[4]
lwr_hg <- hg_ci$percent[3]

# Plot for hg
plot(hg_values, pib_hg, type = "l", 
     xlab = "Height of grasses", ylab = "Predicted % in burned areas",
     main = "Predicted % in burned areas vs height of grasses")
lines(hg_values, upr_hg, col = "red", lty = 2)
lines(hg_values, lwr_hg, col = "red", lty = 2)

# Repeat the bootstrapping process for hf and hs (not shown here, but follow the same logic)
