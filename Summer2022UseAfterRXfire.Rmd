...
grass_height_model <- lmer(max_grasses_cm ~ days_since_fire + (1 | sample_new), data = veg_data_2022)
forb_height_model  <- lmer(max_forbs_cm ~ days_since_fire + (1 | sample_new), data = veg_data_2022)
shrub_height_model <- lmer(max_shrubs_cm ~ days_since_fire + (1 | sample_new), data = veg_data_2022)

# Models for percent cover
grass_cover_model <- lmer(percent_grasses ~ days_since_fire + (1 | sample_new), data = veg_data_2022)
forb_cover_model  <- lmer(percent_forbs ~ days_since_fire + (1 | sample_new), data = veg_data_2022)
shrub_cover_model <- lmer(percent_shrubs ~ days_since_fire + (1 | sample_new), data = veg_data_2022)
...
pred_all_lin_22 <- bind_rows(pred_height_grass, pred_height_forb, pred_height_shrub,
                      pred_cover_grass, pred_cover_forb, pred_cover_shrub)

# Ensure the facet order is as desired
pred_all_lin_22$variable <- factor(pred_all$variable, 
...
model_stats_2022_linear <- bind_rows(
  get_model_stats(grass_height_model) %>% mutate(Model = "Grass Height"),
  get_model_stats(forb_height_model) %>% mutate(Model = "Forb Height"),
  get_model_stats(shrub_height_model) %>% mutate(Model = "Shrub Height"),
  get_model_stats(grass_cover_model) %>% mutate(Model = "Grass Cover"),
  get_model_stats(forb_cover_model) %>% mutate(Model = "Forb Cover"),
  get_model_stats(shrub_cover_model) %>% mutate(Model = "Shrub Cover")
)
...
grass_height_model <- lmer(max_grasses_cm ~ days_since_fire + days_since_fire_sq + (1 | sample_new), data = veg_data_2022)
forb_height_model  <- lmer(max_forbs_cm ~ days_since_fire + days_since_fire_sq + (1 | sample_new), data = veg_data_2022)
shrub_height_model <- lmer(max_shrubs_cm ~ days_since_fire + days_since_fire_sq + (1 | sample_new), data = veg_data_2022)

# Percent cover models
grass_cover_model <- lmer(percent_grasses ~ days_since_fire + days_since_fire_sq + (1 | sample_new), data = veg_data_2022)
forb_cover_model  <- lmer(percent_forbs ~ days_since_fire + days_since_fire_sq + (1 | sample_new), data = veg_data_2022)
shrub_cover_model <- lmer(percent_shrubs ~ days_since_fire + days_since_fire_sq + (1 | sample_new), data = veg_data_2022)
...
# Combine predictions
pred_all_quad_22 <- bind_rows(pred_height_grass, pred_height_forb, pred_height_shrub,
                      pred_cover_grass, pred_cover_forb, pred_cover_shrub)

# Set facet order
pred_all_quad_22$variable <- factor(pred_all$variable,
                            levels = c("Grass Height (cm)", "Forb Height (cm)", "Shrub Height (cm)",
                                       "Grass Cover (%)", "Forb Cover (%)", "Shrub Cover (%)"))
...
model_stats_2022_quad <- bind_rows(
  get_model_stats_quad(grass_height_model) %>% mutate(Model = "Grass Height"),
  get_model_stats_quad(forb_height_model) %>% mutate(Model = "Forb Height"),
  get_model_stats_quad(shrub_height_model) %>% mutate(Model = "Shrub Height"),
  get_model_stats_quad(grass_cover_model) %>% mutate(Model = "Grass Cover"),
  get_model_stats_quad(forb_cover_model) %>% mutate(Model = "Forb Cover"),
  get_model_stats_quad(shrub_cover_model) %>% mutate(Model = "Shrub Cover")
)
...
# Updated plot
ggplot(best_preds_22, aes(x = days_since_fire + 5, y = fit)) +
