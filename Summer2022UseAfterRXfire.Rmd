...
Predict plots with 95% conf int 
```{r}
...
predicted_pib <- predict(bmodel1, newdata = newdata, type = "response")
...
ggplot() +
  geom_point(data = s22rx_uniq, aes(x = DSF, y = pib), size = 2) +  # Raw data points
  geom_line(data = newdata, aes(x = DSF, y = predictedPIB), color = "black") +  # Prediction line
  geom_ribbon(data = newdata, aes(x = DSF, ymin = LowerCI, ymax = UpperCI), alpha = 0.2, fill = "red") +  # Confidence intervals
...
# Add the predicted values to the newdata data frame
newdata$predicted_pib <- predicted_pib  # Ensure this name matches in your ggplot call

# Plot the raw data, prediction line, and confidence intervals
ggplot() +
  geom_point(data = s22rx_uniq, aes(x = DSF, y = pib), size = 2) +  # Raw data points
  geom_line(data = newdata, aes(x = DSF, y = predicted_pib), color = "black") +  # Prediction line
  geom_ribbon(data = newdata, aes(x = DSF, ymin = ci_lower, ymax = ci_upper), alpha = 0.2, fill = "red") +  # Confidence intervals
  labs(x = "Days since fire", y = "Proportion of bobwhite in burned areas") +
  theme_bw() +
  scale_x_continuous(breaks = c(0, 30, 60, 90, 120), limits = c(NA, 120)) +
  theme(legend.position = "none",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.line = element_line(color = "black"),
